...
string UTIL_FileRead( string file, uint32_t start, uint32_t length )
{
	...
	if( IS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] unable to read file part [" + file + "]" , DEBUG_LEVEL_WARN );
		return string( );
	}
...
string UTIL_FileRead( string file )
{
	...
	if( IS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] unable to read file [" + file + "]" , DEBUG_LEVEL_WARN );
		return string( );
	}
...
bool UTIL_FileWrite( string file, unsigned char *data, uint32_t length )
{
	...
	if( OS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] unable to write file [" + file + "]" , DEBUG_LEVEL_WARN);
		return false;
	}
